...
class AddForm(forms.Form):
    ...
    author_choice = forms.ModelChoiceField(label='Existing Authors', queryset=Author.objects.all())
    publisher = forms.CharField(label='Publisher', max_length=50)
    publisher_choice = forms.ModelChoiceField(label='Existing Publishers', queryset=Publisher.objects.all())
    song_type = forms.CharField(label='Song type', max_length=50)
    type_choice=forms.ModelChoiceField(label='Existting Types', queryset=Type.objects.all())
